...
export const RecurringAndRunwayWidget: React.FC = () => {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
      ...
      <div className="lg:col-span-5 bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col justify-between space-y-5">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Net monthly burn & longevity</p>
            </div>
          </div>
        ...
        <div className="bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-4 border border-slate-100 dark:border-slate-800/80">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            Zero-income baseline runway: <span className="font-bold text-slate-700 dark:text-slate-300">{runway.expenseOnlyRunwayMonths} months</span>
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 pt-1 border-t border-slate-100 dark:border-slate-800">
          <div>
            <span className="text-[11px] text-slate-400 font-medium">Avg Monthly Spend</span>
            <p className="text-sm font-bold text-slate-800 dark:text-slate-200 mt-0.5">
              {formatINR(runway.averageMonthlyExpense)}
            </p>
          </div>
          <div>
            <span className="text-[11px] text-slate-400 font-medium">Avg Monthly Inflow</span>
      ...
      <div className="lg:col-span-7 bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col justify-between space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-sm font-bold text-slate-900 dark:text-white">
                Detected Recurring Subscriptions & Bills
              </h3>
              <p className="text-xs text-slate-400">
                Auto-clustered (28-32 day cycle) with one-click user override
              </p>
            </div>
          </div>

          <div className="text-right">
            <span className="text-xs text-slate-400">Fixed Monthly Total: </span>
        ...
        {recurringExpenses.length === 0 && recurringIncomes.length === 0 ? (
          <div className="text-center py-8 bg-slate-50 dark:bg-slate-800/40 rounded-2xl border border-dashed border-slate-200 dark:border-slate-800 p-4">
          ...
          <div className="space-y-2.5 max-h-56 overflow-y-auto pr-1">
            {recurringExpenses.map(item => (
              <div
                key={item.clusterId}
                className="flex items-center justify-between p-3 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-800 hover:border-slate-200 transition-all"
              >
                <div className="flex items-center gap-3 min-w-0">
                  <div className="w-8 h-8 rounded-xl bg-indigo-50 dark:bg-indigo-950/60 text-indigo-600 dark:text-indigo-400 flex items-center justify-center font-bold text-xs flex-shrink-0">
                ...
                <div className="flex items-center gap-3 flex-shrink-0 ml-2">
                  ...
                  <button
                    onClick={() => {
                      // Mark the latest transaction in this cluster as not recurring
            ...
            {recurringIncomes.map(item => (
              <div
                key={item.clusterId}
                className="flex items-center justify-between p-3 rounded-2xl bg-emerald-50/40 dark:bg-emerald-950/20 border border-emerald-100 dark:border-emerald-900/40 hover:border-emerald-200 transition-all"
              >
                <div className="flex items-center gap-3 min-w-0">
